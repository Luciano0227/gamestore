...
function parseTransferCoins(playerId, msg)
	...
	db.query("UPDATE `accounts` SET `premium_points` = `coins` + " .. amount .. " WHERE `id` = " .. accountId)
	...
	if inbox and inbox:getEmptySlots() > 0 then
		...
		local function changeKit(kit)
			...
			if kit then
				...
				kit:setCustomAttribute("ActionId", offerId)
...
function Player.setCoinsBalance(self, coins)
	db.query("UPDATE `accounts` SET `premium_points` = " .. coins .. " WHERE `id` = " .. self:getAccountId())
	return true
end
